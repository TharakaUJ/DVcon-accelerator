...
module system_top #(
    ...
    axi4_lite_slave #(
        ...
        .busy(busy_r), .done(done_r), .error(1'b0), .fsm_state(fsm_state_r)
    );
    ...
    // ── NEW BRAM/DSP control FSM (CR-2 retires LOADW/WPULSE) ──────────────────
        ...
        // pack current input_data vector into a weight row word
        ...
        // Control strobes are COMBINATIONAL decodes of state so they stay aligned
        // with the registered counters (no 1-cycle skew vs wrow/abank/aaddr/...).
        wire             wt_wr_en     = (state == S_WPRELOAD);
        wire             wt_rd_en     = (state == S_WREAD);
        wire             weight_swap_w= (state == S_WSWAP);
        wire             act_wr_en    = (state == S_AWRITE);
        wire             act_rd_en    = (state == S_ASTREAM);
        wire             out_rd_en_w  = (state == S_DRAIN);
        wire [OUT_AW-1:0] out_rd_addr_w = dcnt;
        ...
        always @(posedge clk or negedge rst_n) begin
            if (!rst_n || soft_reset) begin
                ...
                case (state)
                    ...
                    // CR-2: stream the weight tile into BRAM (one row per cycle)
                    S_WPRELOAD:
                        if (wrow == ARRAY_SIZE-1) begin wrow<=0; state<=S_WREAD; end
                        else wrow <= wrow + 1;
                    // Issue the tile read from weight BRAM (data valid next cycle)
                    S_WREAD:  state <= S_WLOAD;
                    // wt_rd_valid high here → array latches the tile into its shadow
                    S_WLOAD:  state <= S_WSWAP;
                    // 1-cycle shadow→active swap (0-stall fast load); shadow ready
                    S_WSWAP:  state <= S_AWRITE;
                    // Stage activations into BRAM: ARRAY_SIZE banks per vector
                    S_AWRITE:
                    ...
                    // Stream activation vectors into the array, one per cycle
                    S_ASTREAM:
                        if (acnt == num_acts-1) begin acnt<=0; state<=S_COMPUTE; end
                        else acnt <= acnt + 1;
                    // Wait for the array + requant/activation pipe to drain
                    S_COMPUTE: if (perf_valid) begin state<=S_DRAIN; dcnt<=0; end
                    // Drain INT8 results out of the output BRAM
                    S_DRAIN:
        ...
        accelerator #(
            ...
            .perf_valid(perf_valid), .perf_cycles(perf_cycles), .result_valid(result_valid),
            // activation BRAM
            .act_wr_en(act_wr_en), .act_wr_buf(abuf), .act_wr_bank(abank[$clog2(ARRAY_SIZE)-1:0]),
            .act_wr_addr(aaddr[ACT_AW-1:0]), .act_wr_data(input_data[abank[$clog2(ARRAY_SIZE)-1:0]]),
            .act_rd_en(act_rd_en), .act_rd_buf(abuf), .act_rd_addr(acnt[ACT_AW-1:0]),
            // weight BRAM
            .wt_wr_en(wt_wr_en), .wt_wr_buf(wbuf), .wt_wr_row(wrow[$clog2(ARRAY_SIZE)-1:0]),
            .wt_wr_row_data(row_pack),
            .wt_rd_en(wt_rd_en), .wt_rd_buf(wbuf), .weight_swap(weight_swap_w),
            // requant / activation
            .vu_bias(vu_bias_w), .vu_requant_mult(cfg_requant_mult),
            .vu_requant_shift(cfg_requant_shift), .vu_act_type(cfg_act_type),
            // output BRAM
            .out_wr_buf(obuf), .out_rd_en(out_rd_en_w), .out_rd_buf(obuf), .out_rd_addr(out_rd_addr_w),
            .out_vec(out_vec), .out_rd_valid(out_rd_valid), .out_wr_en()
        );
