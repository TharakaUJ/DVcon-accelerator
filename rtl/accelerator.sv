...
module accelerator #(
    ...
    // ── LEGACY plain-GEMM bypass ─────────────────────────────────────────────
        ...
        // Legacy register_bank output (registered INT32 flat) — drives output_data
        ...
        register_bank #(.DATA_WIDTH(ARRAY_SIZE*ACCUM_WIDTH), .BUFFER_WIDTH(1)) output_buffer (
            .clk(clk), .rst_n(rst_n), .en(en),
            .register_data_in(flat_compute), .register_data_out(output_data),
            .input_reg_select(1'b0), .output_reg_select(1'b0),
            .connect_wires_out());
