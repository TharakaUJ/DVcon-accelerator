...
module tb_bram_out_buffer;
    ...
    wire signed [DMA_WIDTH-1:0] rd_vec; // UPDATED: Now a packed array
    wire                      rd_valid;
    ...
    task wr(input bf, input [WR_ADDR_W-1:0] ad, input integer base);
        integer ll;
        begin
    ...
    initial begin
        ...
        // buf0 holds +(a*10+l), buf1 holds -(a*10+l)
        // UPDATED: Addresses loop up to OUT_DEPTH / OC_LANES
        for (a=0; a<(OUT_DEPTH/OC_LANES); a=a+1) begin
            wr(1'b0, a[WR_ADDR_W-1:0],  (a*10));
            wr(1'b1, a[WR_ADDR_W-1:0], -(a*10));
        end

        // UPDATED: Validate against packed rd_vec slices using +: part select
        for (a=0; a<(OUT_DEPTH/ELEMENTS_PER_DMA); a=a+1) begin
            @(negedge clk); rd_en=1; rd_buf=1'b0; rd_addr=a[RD_ADDR_W-1:0]; @(posedge clk); #1; rd_en=0;
            check("buf0 rd_valid", rd_valid, 1);
            for (l=0; l<OC_LANES; l=l+1) begin
                check("buf0 vec", $signed(rd_vec[l*DATA_W +: DATA_W]), a*10+l);
            end
        end
        for (a=0; a<(OUT_DEPTH/ELEMENTS_PER_DMA); a=a+1) begin
            @(negedge clk); rd_en=1; rd_buf=1'b1; rd_addr=a[RD_ADDR_W-1:0]; @(posedge clk); #1; rd_en=0;
            for (l=0; l<OC_LANES; l=l+1) begin
                check("buf1 vec", $signed(rd_vec[l*DATA_W +: DATA_W]), -(a*10)+l);
            end
        end
